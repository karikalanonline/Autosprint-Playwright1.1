...
Before({ timeout: 60000 }, async function(this: CustomWorld, scenario) {
  ...
  try {
    ...
    fs.appendFileSync(
      ...
      `Scenario started: ${scenario.pickle.name} at ${new Date().toISOString()}\n`,
      'utf8'
    );  
  } catch (error) {
    console.error(`Failed to setup browser for scenario: ${scenario.pickle.name}`, error);
    throw error;
  }
});
...
After(async function(this: CustomWorld, scenario) {
  if (scenario.result?.status === Status.FAILED && this.page) {
    try {
      ...
      const screenshot = await this.page.screenshot({
        path: path.join(process.cwd(), 'reports', 'screenshots', `${scenario.pickle.name.replace(/[^a-zA-Z0-9]/g, '_')}.png`),
        type: 'png',
        fullPage: true
      });
    ...
    fs.appendFileSync(
      path.join(process.cwd(), 'logs', 'test.log'), 
      `Scenario failed: ${scenario.pickle.name} at ${new Date().toISOString()}\n`,
      'utf8'
    );
  }
  ...
  fs.appendFileSync(
    path.join(process.cwd(), 'logs', 'test.log'), 
    `Scenario completed: ${scenario.pickle.name} at ${new Date().toISOString()}\n`,
    'utf8'
  );
});
